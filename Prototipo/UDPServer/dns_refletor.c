...
//	char dnss[100];
    ...
    qname =(unsigned char*)&buf[sizeof(struct DNS_H)];
 	
    ChangetoDnsNameFormat(qname , host);
    qinfo =(struct QUESTION*)&buf[sizeof(struct DNS_H) + (strlen((const char*)qname) + 1)]; //fill it
    ...
    pkgAnalyzer(dest, buf);
	printAllCounters();
 	printf("\nRecibo: %s\n", (char*) buf);
	
    dns = (struct DNS_H*) buf;
    if( sendto(s,(char*)buf,sizeof(struct DNS_H) + (strlen((const char*)qname)+1) + sizeof(struct QUESTION),0,(struct sockaddr*)&cliente,sizeof(cliente)) < 0)
    {
        perror("sendto cliente failed");
    }
    
    //move ahead of the dns header and the query field
    reader = &buf[sizeof(struct DNS_H) + (strlen((const char*)qname)+1) + sizeof(struct QUESTION)];

 /*    if( sendto(cli,(char*)buf,sizeof(struct DNS_H) + (strlen((const char*)qname)+1) + sizeof(struct QUESTION),0,(struct sockaddr*)&cliente,sizeof(cliente)) < 0)
    {
        perror("sendto failed cliente");
    }*/

/*
    printf("Cliente feito");
