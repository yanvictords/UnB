...
//===================================================== 
	sck_cliente = socket(AF_INET, SOCK_STREAM, 0); //UDP
...
//=== fim das configurações do addr_cliente
	...
	struct sockaddr *cast_addr_cli = (struct sockaddr *) &addr_cli; // cast do endereço para struct genérica
	int tam_addr_cli = sizeof(addr_cli); // tamanho da struct
	int cliente;

	printf("\nEsperando por conexões: TCP...\n");

	listen(sck_cliente, 1);

	printf("\nEscutando na porta %d...\n", _PORTA_CLIENTE);

	if((cliente = accept(sck_cliente, cast_addr_cli, &tam_addr_cli)) == -1) // apos falar, o cliente tenta conexao com o servidor
	{
		perror("A conexao com o cliente nao foi efetuada com sucesso!\n");
	}
	else
		printf("A conexao com o cliente foi efetuada com sucesso!\n");

//====================================
// PROXY ONLINE... ESPERA POR CLIENTE
//====================================
	printf("\n Esperando por mensagens: TCP...\n");
...
//===========================================================
	...
	while(1)
	{
		...
		if((tam_msg = recv(cliente, buffer, LEN, 0)) > 0) // mensagem recebida > 0
		{
			buffer[tam_msg] = '\0';
		...
		//	char ip = inet_ntoa(addr_cliente.sin_addr);
			...
			if(send(sck_servidor, buffer, strlen(buffer), 0)) // repassa o pedido ao servidor
			{
				...
				if((tam_msg = recv(sck_servidor, resposta, LEN, 0)) > 0) // se o servidor responder
				{
					...
					if(send(cliente, resposta, strlen(resposta), 0)) // repassa do servidor ao cliente
